  ...
  return (
    <div className="app-bg min-h-screen">
      <div className="mx-auto min-h-screen max-w-[1600px] lg:grid lg:grid-cols-[260px_1fr]">
        <aside className="fixed inset-y-0 left-0 z-40 hidden w-[260px] flex-col border-r border-white/[.07] bg-[#1E1132]/90 p-5 backdrop-blur-2xl lg:flex xl:left-[max(0px,calc((100vw-1600px)/2))]">
          <div className="flex items-center gap-3 px-2 py-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-[14px] bg-lime-300 text-[#241536] shadow-[0_0_34px_rgba(212,175,127,.18)]"><Timer className="h-5 w-5" strokeWidth={2.5} /></div>
          ...
          <button onClick={() => setScreen('premium')} className="group relative mt-8 overflow-hidden rounded-[1.5rem] border border-lime-300/15 bg-lime-300/[.06] p-4 text-left transition hover:border-lime-300/30 hover:bg-lime-300/[.09]">
            ...
            <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-lime-300 text-[#241536]"><Crown className="h-4 w-4" /></div>
        ...
        <header className="safe-top sticky top-0 z-40 flex items-center justify-between border-b border-white/[.06] bg-[#1A0E2E]/80 px-5 pb-4 backdrop-blur-xl sm:px-8 lg:hidden">
          <div className="flex items-center gap-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-lime-300 text-[#241536] shadow-[0_0_30px_rgba(212,175,127,.14)]">
        ...
        <nav className="safe-bottom fixed bottom-3 left-1/2 z-40 flex w-[calc(100%-1.5rem)] max-w-md -translate-x-1/2 items-center justify-around rounded-[1.4rem] border border-white/10 bg-[#221335]/90 px-2 py-1.5 shadow-[0_20px_60px_rgba(0,0,0,.55)] backdrop-blur-2xl lg:hidden">
          <NavButton
            active={tab === 'home'}
            onClick={() => setTab('home')}
...
function SideNavButton({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    <button onClick={onClick} className={`group flex w-full items-center gap-3 rounded-2xl px-3.5 py-3 text-sm font-semibold transition-all ${active ? 'bg-white/[.075] text-stone-50 shadow-[inset_0_0_0_1px_rgba(255,255,255,.04)]' : 'text-stone-500 hover:bg-white/[.035] hover:text-stone-200'}`}>
      ...
      <span>{label}</span>
      {active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-lime-300 shadow-[0_0_10px_rgba(212,175,127,.7)]" />}
    </button>
  );
}
