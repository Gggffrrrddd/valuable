...
export default function StudyTableScreen({ onBack }: StudyTableScreenProps) {
  ...
  const [plantTransform, setPlantTransform] = useState<ObjectTransform>(PLANT_REFERENCE);
  const [spin, setSpin] = useState(false);
  const plantDragOrigin = useRef<{ x: number; y: number } | null>(null);
    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#090b0a]">
      <div className="absolute inset-0">
        <TableScene3D
          ...
          plantTransforms={[plantTransform]}
          spin={spin}
        />
      </div>
      ...
      <div className="pointer-events-auto absolute left-4 top-20 z-30 w-72 rounded-2xl border border-white/[.08] bg-black/60 p-4 text-stone-300 shadow-2xl backdrop-blur-xl sm:left-6 sm:top-24">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[.18em] text-lime-300">Plant</div>
            <div className="mt-1 text-xs text-stone-400">Single-model tuner — edit and save</div>
          </div>
        </div>
        ...
        <div
          ...
          onPointerDown={(event) => {
            ...
            plantDragOrigin.current = { x: event.clientX, y: event.clientY };
          }}
          onPointerMove={(event) => {
            if (!plantDragOrigin.current) return;
            const dx = event.clientX - plantDragOrigin.current.x;
            const dy = event.clientY - plantDragOrigin.current.y;
            plantDragOrigin.current = { x: event.clientX, y: event.clientY };
            setPlantTransform((current) => ({
              ...current,
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <button
            ...
            className="rounded-lg border border-lime-300/30 px-2.5 py-1.5 text-[10px] font-bold text-lime-200 hover:bg-lime-300/10"
            onClick={() => {
              console.log('PLANT_START');
              console.log(JSON.stringify(plantTransform, null, 2));
              console.log('PLANT_END');
            }}
          >
            Save
          </button>
          ...
          {SLIDER_ROWS.map(({ key, label, min, max, step }) => (
            <label key={key} className="grid grid-cols-[4.5rem_1fr_2.5rem] items-center gap-2 text-[10px]">
              ...
              <input
                ...
                max={max}
                step={step}
                value={plantTransform[key]}
                onChange={(event) => {
                  const value = Number(event.target.value);
                  setPlantTransform((current) => ({ ...current, [key]: value }));
                }}
                className="h-1 accent-lime-300"
              />
              <NumericInput
                value={plantTransform[key]}
                onCommit={(value) => setPlantTransform((current) => ({ ...current, [key]: value }))}
                min={min}
                max={max}
              />
            </label>
