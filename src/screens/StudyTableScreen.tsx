...
export default function StudyTableScreen({ onBack }: StudyTableScreenProps) {
  ...
  /** 5× closer camera: divide default distance/height by 5. */
  const zoomedTableTransform = useMemo(
    () => ({
      ...DEFAULT_TABLE_TRANSFORM,
      cameraDistance: DEFAULT_TABLE_TRANSFORM.cameraDistance / 5,
      cameraHeight: DEFAULT_TABLE_TRANSFORM.cameraHeight / 5,
    }),
    [],
  );
    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#090b0a]">
      <div className="absolute inset-0">
        <TableScene3D
          ...
          transform={zoomedTableTransform}
          chairs={CHAIR_SEATS}
      ...
      <div className="pointer-events-auto absolute right-4 top-20 z-30 w-72 rounded-2xl border border-white/[.08] bg-black/60 p-4 text-stone-300 shadow-2xl backdrop-blur-xl sm:right-6 sm:top-24">
        <div className="mb-3">
          ...
          <div className="mt-1 text-xs text-stone-400">Move, size and tilt — Save logs all values</div>
        </div>
