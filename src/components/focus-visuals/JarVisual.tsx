...
const WATER_CALIBRATION_URL = '/visuals/jar/jar-water-calibration.png';
const WATER_CALIBRATION_KEY = 'valuable-jar-water-calibration';
...
const DEFAULT_CALIBRATION: WaterCalibration = { x: 590, y: 500, width: 270, height: 180 };
...
export default function JarVisual({ progress }: FocusVisualProps) {
  ...
  const saveCalibration = () => {
    ...
    console.log('[JarWaterCalibration:SAVED]', JSON.stringify(calibration));
  };
    ...
    <div ref={containerRef} className={`focus-visual ${complete ? 'visual-complete' : ''}`} style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }} role="img" aria-label={`Water jar ${Math.round(value * 100)} percent complete`}>
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${viewport.width} ${viewport.height}`} preserveAspectRatio="none" aria-hidden="true">
        ...
        <g transform={sceneTransform}>
          ...
          {/* Hidden during calibration so the reference image can be aligned cleanly. */}
          {!WATER_CALIBRATION_MODE && <g clipPath="url(#jar-interior-mask)">
          ...
          {!WATER_CALIBRATION_MODE && <g clipPath="url(#jar-interior-mask)" style={dripVariables}>
      ...
      {WATER_CALIBRATION_MODE && <div style={{ position: 'absolute', right: 18, top: 18, zIndex: 20, width: 250, padding: 14, border: '1px solid rgba(217,255,100,.55)', borderRadius: 12, background: 'rgba(12,16,10,.88)', color: '#f5f5f4', fontFamily: 'ui-monospace, monospace', fontSize: 12, boxShadow: '0 14px 40px rgba(0,0,0,.35)' }}>
        <strong style={{ display: 'block', color: '#d9ff64', marginBottom: 9 }}>Jar water calibration</strong>
