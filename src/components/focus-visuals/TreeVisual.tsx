...
type LeafState = 'falling' | 'landed';
type LeafStyle = CSSProperties & Record<`--leaf-${string}`, string | number>;

const FALL_DURATION_SECONDS = 2.6;
const MAX_FALLING_LEAVES = 4;
const GROUND_Y = 0.84;
...
const SHED_ORDER = [...LEAVES].sort((a, b) => seededUnit(a.id) - seededUnit(b.id));
const LANDINGS = new Map(LEAVES.map((leaf) => {
  const horizontal = seededUnit(leaf.id + 101);
  const depth = seededUnit(leaf.id + 211);
  const turn = seededUnit(leaf.id + 307);
  return [leaf.id, {
    x: 0.16 + horizontal * 0.33,
    y: GROUND_Y + depth * 0.035,
    rotation: leaf.rotation + (turn < 0.5 ? -1 : 1) * (10 + turn * 20),
    sway: 16 + seededUnit(leaf.id + 401) * 22,
    direction: seededUnit(leaf.id + 503) < 0.5 ? -1 : 1,
  }];
}));
...
export default function TreeVisual({ progress, duration, running }: TreeVisualProps) {
  ...
  const [leafStates, setLeafStates] = useState<Record<number, LeafState>>({});
  const queueRef = useRef<number[]>([]);
  const scheduledCountRef = useRef(0);
  const fallingCountRef = useRef(0);

  const startQueuedLeaves = useCallback(() => {
    const slots = MAX_FALLING_LEAVES - fallingCountRef.current;
    if (slots <= 0 || queueRef.current.length === 0) return;

    const starting = queueRef.current.splice(0, slots);
    fallingCountRef.current += starting.length;
    setLeafStates((current) => {
      const next = { ...current };
      starting.forEach((id) => { next[id] = 'falling'; });
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (complete) {
      queueRef.current = [];
      scheduledCountRef.current = LEAVES.length;
      fallingCountRef.current = 0;
      setLeafStates(Object.fromEntries(LEAVES.map((leaf) => [leaf.id, 'landed' as const])));
      return;
    }

    const targetCount = Math.min(LEAVES.length, Math.floor(progress * LEAVES.length));
    if (targetCount <= scheduledCountRef.current) return;

    const newlyScheduled = SHED_ORDER
      .slice(scheduledCountRef.current, targetCount)
      .map((leaf) => leaf.id);
    scheduledCountRef.current = targetCount;

    if (reducedMotion) {
      setLeafStates((current) => {
        const next = { ...current };
        newlyScheduled.forEach((id) => { next[id] = 'landed'; });
        return next;
      });
      return;
    }

    queueRef.current.push(...newlyScheduled);
    startQueuedLeaves();
  }, [activeSession, complete, progress, reducedMotion, startQueuedLeaves]);

  const handleFallComplete = useCallback((event: AnimationEvent<HTMLSpanElement>, leafId: number) => {
    if (event.target !== event.currentTarget) return;
    fallingCountRef.current = Math.max(0, fallingCountRef.current - 1);
    setLeafStates((current) => ({ ...current, [leafId]: 'landed' }));
    requestAnimationFrame(startQueuedLeaves);
  }, [startQueuedLeaves]);
  ...
  return (
    ...
    >
      ...
      {LEAVES.map((leaf) => {
        const state = activeSession ? leafStates[leaf.id] : undefined;
        const landing = LANDINGS.get(leaf.id)!;
        const verticalDistance = landing.y - leaf.y;
        const style: LeafStyle = {
          left: `${(state === 'landed' ? landing.x : leaf.x) * 100}%`,
          top: `${(state === 'landed' ? landing.y : leaf.y) * 100}%`,
          transform: `translate(-50%, -50%) rotate(${state === 'landed' ? landing.rotation : leaf.rotation}deg) scale(${leaf.scale})`,
          filter: `hue-rotate(${leaf.hue}deg) brightness(${leaf.brightness})`,
          '--leaf-start-x': `${leaf.x * 100}%`,
          '--leaf-start-y': `${leaf.y * 100}%`,
          '--leaf-land-x': `${landing.x * 100}%`,
          '--leaf-land-y': `${landing.y * 100}%`,
          '--leaf-y-14': `${(leaf.y + verticalDistance * 0.14) * 100}%`,
          '--leaf-y-36': `${(leaf.y + verticalDistance * 0.36) * 100}%`,
          '--leaf-y-62': `${(leaf.y + verticalDistance * 0.62) * 100}%`,
          '--leaf-y-86': `${(leaf.y + verticalDistance * 0.86) * 100}%`,
          '--leaf-y-bounce': `${(landing.y - 0.007) * 100}%`,
          '--leaf-y-settle': `${(landing.y + 0.0018) * 100}%`,
          '--leaf-start-rotation': `${leaf.rotation}deg`,
          '--leaf-land-rotation': `${landing.rotation}deg`,
          '--leaf-scale': leaf.scale,
          '--leaf-sway': `${landing.sway * landing.direction}px`,
          '--leaf-sway-back': `${landing.sway * landing.direction * -0.72}px`,
          '--leaf-sway-forward': `${landing.sway * landing.direction * 0.52}px`,
          '--leaf-sway-land': `${landing.sway * landing.direction * -0.25}px`,
          '--leaf-fall-duration': `${FALL_DURATION_SECONDS}s`,
        ...
        return (
          <span
            ...
            key={leaf.id}
            aria-hidden="true"
            onAnimationEnd={state === 'falling' ? (event) => handleFallComplete(event, leaf.id) : undefined}
          >
